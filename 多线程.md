...
暂时还没有找到比较好的资料,所以整理的可能没有那么好。
首先我们先给出会出现死锁的原因,很显然,正如上面所说,如果生产者占用着公共存储空间,那么当存储空间满的时候,他依旧占用,但他不能够继续往下执行下去,那么因为消费者不能够占有公共存储空间,那么就会让存储空间处于满的状态,也就是两个想成都无法执行下去。
下面我找的代码示例是我认为比较好的:
```java
//生产者
...
public class BoxDemo {
    public static void main(String[] args) {
        ...
        Thread c1 = new Thread(new Customer(b));
        Thread c2 = new Thread(new Customer(b));
        Thread c3 = new Thread(new Customer(b));
